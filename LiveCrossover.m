classdef (StrictDefaults)LiveCrossover < matlab.System & audioPlugin
    ...
    methods
        ...
        function set.Slope(plugin,val)
            plugin.Slope = min(6*round(val/6),48);
            needToDesignFilters(plugin);
        end
    ...
    methods (Access = private)
        function calculateCoefficients(plugin)
            Fs = getSampleRate(plugin);
            [B1,A1] = designVarSlopeFilter(plugin.Slope,plugin.Cutoff/(Fs/2),'hi','Orientation','row');
            plugin.Numerator = [B1];
            plugin.Denominator = [A1];
